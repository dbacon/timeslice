...
public class Bucket<T, K, V>
{
    ...
    static <T1, K1> Bucket<T1, K1, T1> create(ITransform<T1, K1> keyMaker)
    {
        return create(keyMaker, Transforms.<T1>identity());
    }

    static <T1, K1, V1> Bucket<T1, K1, V1> create(ITransform<T1, K1> keyMaker, ITransform<T1, V1> valueMaker)
    {
        return new Bucket<T1, K1, V1>(keyMaker, valueMaker);
    }

    Bucket(ITransform<T, K> keyMaker, ITransform<T, V> valueMaker)
    {
