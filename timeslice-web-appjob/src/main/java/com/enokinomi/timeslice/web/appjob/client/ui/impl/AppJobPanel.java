...
public class AppJobPanel extends ResizeComposite implements IAppJobPanel
{
    ...
    private final ListenerManager<IAppJobPanelListener> listenerMgr = new ListenerManager<IAppJobPanelListener>();
    @Override public Registration addListener(IAppJobPanelListener listener) { return listenerMgr.addListener(listener); }
    protected void fireAppJobRequested(String jobId) { for (IAppJobPanelListener listener: listenerMgr.getListeners()) { listener.appJobRequested(jobId); } }
    protected void fireJobListRefreshRequested() { for (IAppJobPanelListener listener: listenerMgr.getListeners()) { listener.appJobListRefreshRequested(); } }
